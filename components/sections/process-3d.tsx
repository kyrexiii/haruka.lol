...
export default function Process3D() {
  return (
    <section className="bg-[#111111] border border-foreground/5 rounded-3xl md:rounded-[40px] p-8 md:p-20 overflow-hidden relative group/process">
      ...
      <div className="flex flex-col items-center justify-center min-h-[600px] relative" style={{ perspective: "2000px" }}>
        {stages.map((stage, i) => (
          <motion.div 
            key={i} 
            className="absolute w-[80%] max-w-[600px] h-40 md:h-48 bg-card/90 border border-foreground/[0.08] rounded-[32px] p-8 md:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.5),inset_0_1px_2px_rgba(255,255,255,0.05)] cursor-pointer"
          ...
          >
            <div className="absolute top-4 left-4 text-[10px] font-mono text-foreground/30 tracking-widest">{stage.id}</div>
            <div className="flex items-center gap-6 h-full">
               <div className="w-16 h-16 bg-[#18181b] border border-foreground/[0.1] rounded-2xl flex items-center justify-center shadow-2xl">
                 <stage.icon className={`w-8 h-8 ${stage.color}`} />
               </div>
               <div className="space-y-1">
                 <h3 className="text-2xl font-bold text-foreground tracking-tight">{stage.title}</h3>
                 <p className="text-foreground/40 text-sm max-w-[280px]">{stage.desc}</p>
               </div>
            </div>
          </motion.div>
        ))}
